      ...
      <TabsList className="grid w-full grid-cols-3">
        <TabsTrigger value="lexicon" className="p-0">
          <Link
            href={
              wordId
                ? `/language/${conlang.id}/?view=lexicon&word=${wordId}`
                : `/language/${conlang.id}/?view=lexicon`
            }
        ...
        <TabsTrigger value="phonology" className="p-0">
          <Link
            href={
              wordId
                ? `/language/${conlang.id}/?view=phonology&word=${wordId}`
                : `/language/${conlang.id}/?view=phonology`
            }
            className="h-full w-full px-3 py-1.5"
          >
            Phonology
        ...
        <TabsTrigger value="grammar" className="p-0">
          <Link
            href={
              wordId
                ? `/language/${conlang.id}/?view=grammar&word=${wordId}`
                : `/language/${conlang.id}/?view=grammar`
            }
            className="h-full w-full px-3 py-1.5"
          >
            Grammar
